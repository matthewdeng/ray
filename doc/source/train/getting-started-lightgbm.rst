...
.. testcode:: python
    ...
    config = {"label_column": "y", "num_boost_round": 100}
...
Convert this into a native `lightgbm.Dataset <https://lightgbm.readthedocs.io/en/stable/Python-API.html#lightgbm.Dataset>`_.


.. testcode:: python
    :skipif: True

    def get_lgb_dataset(dataset_name: str) -> lightgbm.Dataset:
    ...
    def train_func():
        dtrain = get_lgb_dataset("train")
        deval = get_lgb_dataset("eval")
        ...
...
For more details on LightGBM's GPU support, see the `LightGBM GPU documentation <https://lightgbm.readthedocs.io/en/stable/GPU-Tutorial.html>`__.
...
with a :class:`~ray.train.lightgbm.LightGBMTrainer`.
